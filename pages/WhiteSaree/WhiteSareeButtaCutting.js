...
export default function WhiteSareeButtaCutting({ navigation }) {
  ...
  useEffect(() => {
    (async () => {
      try {
        ...
        const data = await jecardFirebaseUtils.loadButtaWhiteRows(tenantId);
        setRows(data);
      } catch {
        setRows([]);
      }
  ...
  const handleSendToBleach = async (row) => {
    try {
      const tenantId = userData?.companyId || userData?.company?.id || userData?.companyName || 'default';
      
      // Mark as sent to bleach
      ...
      // Save to Firebase
      await jecardFirebaseUtils.saveButtaWhiteRows(updatedRows, tenantId);
      
      // Add to Bleach workflow
      const bleachData = await jecardFirebaseUtils.loadBleachWhiteRows(tenantId);
      await jecardFirebaseUtils.saveBleachWhiteRows([...bleachData, { ...row, sentToBleach: true }], tenantId);
      
      setRows(updatedRows);
      Alert.alert('Success', 'Item sent to Bleach successfully!');
    } catch (error) {
      console.error('Error sending to bleach:', error);
          ...
          <View style={styles.modalCard}>
            ...
            <TouchableOpacity style={[styles.downloadBtn, { marginTop: 14 }]} activeOpacity={0.9} onPress={async () => {
              ...
              try {
                ...
                const bleachCounterRef = await AsyncStorage.getItem('bleach_challan_counter_white');
                const nextNo = bleachCounterRef ? parseInt(bleachCounterRef) + 1 : 1;
                await AsyncStorage.setItem('bleach_challan_counter_white', String(nextNo));
                
                const filename = `W Bleach - ${String(nextNo)}.pdf`;
