...
export default function ColorSareeCotting({ navigation }) {
  ...
  useEffect(() => {
    (async () => {
      try {
        ...
        const data = await jecardFirebaseUtils.loadCottingColorRows(tenantId);
        setRows(data);
      } catch {
        setRows([]);
      }
  ...
  const pendingRows = rows.filter(r => !r.sentToPositionPrint);
  const doneRows = rows.filter(r => r.sentToPositionPrint);
              ...
              <TouchableOpacity
                ...
                onPress={async () => {
                  try {
                    ...
                    const enriched = { ...selectedRow, clientName, chalanNo, piece: pieceVal, mtr: mtrVal, sentToPositionPrint: true };

                    // Update cotting rows - mark as sent
                    const updatedRows = rows.map(r => 
                      (r.poNo === selectedRow.poNo && r.designNo === selectedRow.designNo) 
                        ? enriched 
                        : r
                    );
                    await jecardFirebaseUtils.saveCottingColorRows(updatedRows, tenantId);

                    // Add to Position Print workflow
                    const positionPrintData = await jecardFirebaseUtils.loadPositionPrintColorRows(tenantId);
                    const newPositionPrintRow = {
                      poNo: enriched.poNo,
                    ...
                    await jecardFirebaseUtils.savePositionPrintColorRows([...positionPrintData, newPositionPrintRow], tenantId);

                    setRows(updatedRows);
                    setShowPreview(false);
                    setSelectedRow(null);
                    setActiveTab('done');
                    navigation?.navigate?.('ColorSareePositionPrint');
                  } catch (error) {
          ...
          <View style={styles.modalCard}>
            ...
            <TouchableOpacity style={[styles.downloadBtn, { marginTop: 14 }]} activeOpacity={0.9} onPress={async () => {
              ...
              try {
                ...
                // Get next Position Print counter
                const positionPrintCounterRef = await AsyncStorage.getItem('positionprint_challan_counter_color');
                const nextNo = positionPrintCounterRef ? parseInt(positionPrintCounterRef) + 1 : 1;
                await AsyncStorage.setItem('positionprint_challan_counter_color', String(nextNo));
                
                const filename = `C Position Print - ${String(nextNo)}.pdf`;
...
const styles = StyleSheet.create({
  ...
  previewOverlay: { position: 'absolute', top: 0, left: 0, right: 0, bottom: 0, backgroundColor: 'rgba(0,0,0,0.85)', justifyContent: 'center', alignItems: 'center', zIndex: 1000 },
  previewBox: { backgroundColor: '#2A2A2A', width: '90%', maxHeight: '80%', borderRadius: 16, padding: 16, borderWidth: 1, borderColor: '#555555' },
  ...
  previewTableHeader: { flexDirection: 'row', backgroundColor: '#1A1A1A', borderRadius: 8, paddingVertical: 10, paddingHorizontal: 8, marginTop: 8, borderWidth: 1, borderColor: '#333333' },
  previewHeaderCell: { flex: 1, fontSize: 11, fontWeight: '700', color: '#00BFFF', textAlign: 'center' },
  previewTableRow: { flexDirection: 'row', backgroundColor: '#3A3A3A', borderRadius: 8, paddingVertical: 12, paddingHorizontal: 8, marginTop: 6, borderWidth: 1, borderColor: '#555555' },
  previewBodyCell: { flex: 1, color: '#FFFFFF', fontSize: 12, textAlign: 'center', fontWeight: '600' },
  textInput: { backgroundColor: '#3A3A3A', borderRadius: 8, paddingVertical: 10, paddingHorizontal: 12, color: '#FFFFFF', fontSize: 14, borderWidth: 1, borderColor: '#555555' },
});
