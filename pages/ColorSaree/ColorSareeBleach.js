...
export default function ColorSareeBleach({ navigation }) {
  ...
  useEffect(() => {
    (async () => {
      try {
        ...
        const data = await jecardFirebaseUtils.loadBleachColorRows(tenantId);
        setRows(data);
      } catch {
        setRows([]);
      }
  ...
  return (
    <View style={styles.container}>
      <View style={styles.header}>
        ...
        <Text style={styles.headerTitle}>Color Saree - Bleach</Text>
        <View style={{ width: 24 }} />
      </View>
              ...
              <TouchableOpacity
                ...
                onPress={async () => {
                  try {
                    ...
                    await jecardFirebaseUtils.saveBleachColorRows(updatedRows, tenantId);

                    // Add to Cotting workflow
                    const cottingData = await jecardFirebaseUtils.loadCottingColorRows(tenantId);
                    const newCottingRow = {
                      poNo: enriched.poNo,
                    ...
                    await jecardFirebaseUtils.saveCottingColorRows([...cottingData, newCottingRow], tenantId);

                    setRows(updatedRows);
                    setShowPreview(false);
                    setSelectedRow(null);
                    setActiveTab('done');
                    navigation?.navigate?.('ColorSareeCotting');
                  } catch (error) {
          ...
          <View style={styles.modalCard}>
            ...
            <TouchableOpacity style={[styles.downloadBtn, { marginTop: 14 }]} activeOpacity={0.9} onPress={async () => {
              ...
              try {
                ...
                const cottingCounterRef = await AsyncStorage.getItem('cotting_challan_counter_color');
                const nextNo = cottingCounterRef ? parseInt(cottingCounterRef) + 1 : 1;
                await AsyncStorage.setItem('cotting_challan_counter_color', String(nextNo));
                
                const filename = `C Cotting - ${String(nextNo)}.pdf`;
