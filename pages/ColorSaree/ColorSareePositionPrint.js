...
export default function ColorSareePositionPrint({ navigation }) {
  ...
  useEffect(() => {
    (async () => {
      try {
        ...
        const data = await jecardFirebaseUtils.loadPositionPrintColorRows(tenantId);
        setRows(data);
      } catch {
        setRows([]);
      }
  ...
  const pendingRows = rows.filter(r => !r.sentToChecking);
  const doneRows = rows.filter(r => r.sentToChecking);
              ...
              <TouchableOpacity
                ...
                onPress={async () => {
                  try {
                    ...
                    const enriched = { ...selectedRow, clientName, chalanNo, piece: pieceVal, mtr: mtrVal, sentToChecking: true };

                    // Update position print rows - mark as sent
                    const updatedRows = rows.map(r => 
                      (r.poNo === selectedRow.poNo && r.designNo === selectedRow.designNo) 
                        ? enriched 
                        : r
                    );
                    await jecardFirebaseUtils.savePositionPrintColorRows(updatedRows, tenantId);

                    // Add to Checking workflow
                    const checkingData = await jecardFirebaseUtils.loadCheckingColorRows(tenantId);
                    const newCheckingRow = {
                      poNo: enriched.poNo,
                    ...
                    await jecardFirebaseUtils.saveCheckingColorRows([...checkingData, newCheckingRow], tenantId);

                    setRows(updatedRows);
                    setShowPreview(false);
                    setSelectedRow(null);
                    setActiveTab('done');
                    navigation?.navigate?.('ColorSareeChecking');
                  } catch (error) {
          ...
          <View style={styles.modalCard}>
            ...
            <TouchableOpacity style={[styles.downloadBtn, { marginTop: 14 }]} activeOpacity={0.9} onPress={async () => {
              ...
              try {
                ...
                // Get next Checking counter
                const checkingCounterRef = await AsyncStorage.getItem('checking_challan_counter_color');
                const nextNo = checkingCounterRef ? parseInt(checkingCounterRef) + 1 : 1;
                await AsyncStorage.setItem('checking_challan_counter_color', String(nextNo));
                
                const filename = `C Checking - ${String(nextNo)}.pdf`;
