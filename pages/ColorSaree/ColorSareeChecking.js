...
export default function ColorSareeChecking({ navigation }) {
  ...
  useEffect(() => {
    (async () => {
      try {
        ...
        const data = await jecardFirebaseUtils.loadCheckingColorRows(tenantId);
        setRows(data);
      } catch {
        setRows([]);
      }
  ...
  return (
    <View style={styles.container}>
      <View style={styles.header}>
        ...
        <Text style={styles.headerTitle}>Color Saree - Checking</Text>
        <View style={{ width: 24 }} />
      </View>
              ...
              <TouchableOpacity
                ...
                onPress={async () => {
                  try {
                    ...
                    const enriched = { ...selectedRow, clientName, chalanNo, piece: pieceVal, mtr: mtrVal, sentToDelivery: true };

                    // Update checking rows - mark as sent
                    const updatedRows = rows.map(r => 
                      (r.poNo === selectedRow.poNo && r.designNo === selectedRow.designNo) 
                        ? enriched 
                        : r
                    );
                    await jecardFirebaseUtils.saveCheckingColorRows(updatedRows, tenantId);

                    // Add to Delivery workflow
                    const deliveryData = await jecardFirebaseUtils.loadDeliveryColorRows(tenantId);
                    ...
                    await jecardFirebaseUtils.saveDeliveryColorRows([...deliveryData, newDeliveryRow], tenantId);

                    setRows(updatedRows);
                    setShowPreview(false);
                    setSelectedRow(null);
                    setActiveTab('done');
                    navigation?.navigate?.('ColorSareeDelivery');
                  } catch (error) {
          ...
          <View style={styles.modalCard}>
            ...
            <TouchableOpacity style={[styles.downloadBtn, { marginTop: 14 }]} activeOpacity={0.9} onPress={async () => {
              ...
              try {
                ...
                const deliveryCounterRef = await AsyncStorage.getItem('delivery_challan_counter_color');
                const nextNo = deliveryCounterRef ? parseInt(deliveryCounterRef) + 1 : 1;
                await AsyncStorage.setItem('delivery_challan_counter_color', String(nextNo));
                
                const filename = `C Delivery - ${String(nextNo)}.pdf`;
